...
import value from '../public/examples.json'

const Editor = dynamic(
    import ('./Editor'), { ssr: false })
const SEditor = dynamic(
    import ('./Split-Editor'), { ssr: false })
    ...
    render() {
        return ( <
            ...
            div className = "column col-12" > {
                this.state.error ? ( <
                    div className = "toast toast-error" > { `There is an error: ${this.state.error.toString()}` } <
                    /div>
                ) : ( <
                    div className = "toast toast-success" > { `Text is valid!!` } <
                    /div>
                )
            } <
            /div> <
            ...
            div className = "pure-u-1" >
            <
            SEditor value = { this.state.value }
