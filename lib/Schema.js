...
var Schema = function () {
    ...
    _createClass(Schema, [{
        ...
        value: function generateField(config) {
            ...
            if (config.faker) {
                return (0, _utils.stringToFn)('faker', config.faker, object, db);
            } else if (config.chance) {
                return (0, _utils.stringToFn)('chance', config.chance, object, db);
            } else if (config.casual) {
                return (0, _utils.stringToFn)('casual', config.casual, object, db);
            } else if (config.randexp) {
                return (0, _utils.randexpWrapper)(config.randexp);
            } else if (config.self) {
                return (0, _utils.stringToFn)('object', config.self, object, db);
            } else if (config.db) {
                return (0, _utils.stringToFn)('db', config.db, object, db);
            } else if (config.related) {
                var entities = this.db[config.related];
                var i = Math.floor(entities.length * Math.random());

                if (!config.get) {
                    return entities[i];
                } else {
                    var entity = entities[i];
                    return (0, _utils.stringToFn)('object', config.get, entity, db);
                }
            } else if (config.values) {
                var i = Math.floor(config.values.length * Math.random());
                return config.values[i];
            } else if (config.function) {
                return (0, _utils.fnCallWithContext)(config.function, object, db);
