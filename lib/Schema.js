...
var Schema = function (_Generator) {
    ...
    function Schema(name, cfg, options) {
        ...
        var _this2 = _possibleConstructorReturn(this, (Schema.__proto__ || Object.getPrototypeOf(Schema)).call(this));
    ...
    _createClass(Schema, [{
        ...
        value: function proccessLeaf(field) {
            ...
            if ((0, _utils.isArray)(field)) {
                ...
                if (fieldConfig.concat) {
                    na = (0, _utils.evalWithContextData)(fieldConfig.concat, this.object, this.DB);
                    //Strict Mode
                    na = fieldConfig.concatStrict ? [].concat(_toConsumableArray(new Set(na))) : na;
                }

                var length = (0, _utils.fieldArrayCalcLength)(fieldConfig, na.length, this);

                Array.from(new Array(length)).map(function (el, index) {
                    array.push(_this3.generateField(fieldConfig, index));
                });
            ...
            generators.map(function (key) {
                try {
                    if (cfg.hasOwnProperty(key)) {
                        result = _this4[key].apply(_this4, [cfg].concat(args));
                    }
                } catch (e) {
                    result = null;
        ...
        value: function build(db) {
            ...
            if (Number.isInteger(this.options)) {
                ...
                var f = this.options.uniqueField;
                var entityConfig = this.schema;
                var possibleValues = void 0;
                if (f === '.') {
                    possibleValues = this.schema.values;
                } else {
                    if (this.schema[f]) {
                        if ((0, _utils.isArray)(this.schema[f].values)) {
